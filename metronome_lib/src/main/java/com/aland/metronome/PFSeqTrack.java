...
import static com.aland.metronome.PFSeqService.LOG_TAG;
import static com.aland.metronome.PFSeqConfig.BUFFER_SIZE_BYTES;
...
import static com.aland.metronome.PFSeqConfig.SYNC_MARGIN_MILLIS;
...
public class PFSeqTrack {
    ...
    public PFSeqTrack(PFSeqService seq, String name) {
        this.seq = (PFSeqService) seq;
        this.name = name;
    ...
    boolean initializeAT() {
        ...
        sampleRate = seq.getConfig().getInt(SAMPLE_RATE);
...
//                                    long audioLatencyUs = (Integer) getLatencyMethod.invoke(audioTrack, (Object[]) null) * 1000L - C.TIME_UNSET;
//
//                                    Log.d(LOG_TAG, "audio latency: " + audioLatencyUs);
//                                    atStartNanotime = framesToDurationUs(audioTrack.getPlaybackHeadPosition() - audioLatencyUs;
//
//                                    Log.d(LOG_TAG, "audio atStartNanotime: " + atStartNanotime);
//                                }
//
//                            } catch (Exception e) {
//                                e.printStackTrace();
...
//                        }
        ...
        final short[] preliminarySilence = seq.makeSilence(getSeq().nanoToFrames(getSeq().getConfig().getInt(MIN_MILLIS_AHEAD_TO_WRITE) * (int) PFSeqService.NANO_PER_MILLIS));
        Runnable runnable = new Runnable() {
            @Override
            public void run() {
...
//        Log.d(LOG_TAG, "write locked: " + (isWriteLocked.get() ? "true" : "false"));
    ...
    private long framesToDurationUs(long frameCount) {
        return (frameCount * NANOS_PER_SECOND) / sampleRate;
    }
    ...
    public boolean enoughWrittenToSync() {
        ...
        long syncMarginNano = getSeq().getConfig().getInt(SYNC_MARGIN_MILLIS) * (long) PFSeqService.NANO_PER_MILLIS;
        long nanoWeWantWrittenUntil = System.nanoTime() + (getSeq().getConfig().getInt(MIN_MILLIS_AHEAD_TO_WRITE) * (long) PFSeqService.NANO_PER_MILLIS);
