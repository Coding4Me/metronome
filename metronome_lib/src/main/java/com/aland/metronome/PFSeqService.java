...
import static com.aland.metronome.PFSeqConfig.CONTROL_THREAD_POLLING_MILLIS;
import static com.aland.metronome.PFSeqConfig.FADE_LENGTH_FRAMES;
import static com.aland.metronome.PFSeqConfig.FRAMES_TO_LEAVE_BEFORE_NEXT_ITEM;
import static com.aland.metronome.PFSeqConfig.MAX_BPM;
import static com.aland.metronome.PFSeqConfig.MAX_TRACKS;
import static com.aland.metronome.PFSeqConfig.MIN_BPM;
import static com.aland.metronome.PFSeqConfig.MIN_MILLIS_AHEAD_TO_WRITE;
import static com.aland.metronome.PFSeqConfig.MIN_WRITABLE_CONTENT_NANO;
import static com.aland.metronome.PFSeqConfig.ONGOING_NOTIF_ID;
import static com.aland.metronome.PFSeqConfig.RUN_IN_FOREGROUND;
import static com.aland.metronome.PFSeqConfig.SAMPLE_RATE;
import static com.aland.metronome.PFSeqConfig.SMALLEST_STOPGAP_SILENCE_MILLIS;
import static com.aland.metronome.PFSeqConfig.SYNC_POLLING_SLEEP_MILLIS;
import static com.aland.metronome.PFSeqConfig.SYNC_TIME_OUT_MILLIS;
import static com.aland.metronome.PFSeqConfig.TEMPO;
import static com.aland.metronome.PFSeqConfig.TIMESTAMP_POLLING_DELAY_MILLIS;
import static com.aland.metronome.PFSeqMessage.MESSAGE_TYPE_ALERT;
import static com.aland.metronome.PFSeqMessage.MESSAGE_TYPE_ERROR;


public class PFSeqService extends Service {
    ...
    private IPFSeqService.Stub ipfSeqService;
    ...
    public IBinder onBind(Intent intent) {
        if (ipfSeqService == null) {
            ipfSeqService = new IPFSeqService.Stub() {
                @Override
                public boolean init(String file, PFSeqConfig config) throws RemoteException {

                    if (config == null) {
                        config = new PFSeqConfig(null, null, null, null);
                    }

                    if (!config.isValid()) {
                        throw new RemoteException("PFSeqConfig not valid");
                    }

                    PFSeqService.this.setUpSequencer(config);

                    if (TextUtils.isEmpty(file)) {
                        throw new RemoteException("audio file not valid\"");
                    }
                    PFSeqTrack metronomeTrack = new PFSeqTrack(PFSeqService.this, "metronome");
                    PFSeqClip clip = new PFSeqClip(PFSeqService.this, new File(file));

                    PFSeqTimeOffset timeOffset = PFSeqTimeOffset.make(0, PFSeqTimeOffset.MODE_FRACTIONAL, 0, 4, 0, false, 0);
                    PFSeqPianoRollItem item1 = new PFSeqPianoRollItem(PFSeqService.this, clip, "item 1", timeOffset);
                    metronomeTrack.addPianoRollItem(item1);

                    PFSeqService.this.addTrack(metronomeTrack);
                ...
                public void play() throws RemoteException {
                    PFSeqService.this.play();
                }

                @Override
                public boolean isPlaying() throws RemoteException {
                    return PFSeqService.this.isPlaying();
                }

                @Override
                public void stop() throws RemoteException {

                    PFSeqService.this.stop();
                }

                @Override
                public void setBmp(double newBmp) throws RemoteException {

                    PFSeqService.this.setBpm(newBmp);
                }

                @Override
                public double getBmp() throws RemoteException {

                    return PFSeqService.this.getBpm().doubleValue();
                }

                @Override
                public void registerListener(IServiceCallback listener) {
                    PFSeqService.this.listener = listener;
                }
            };
        }
        return ipfSeqService;
    }
    ...
    private boolean setUpSequencer(PFSeqConfig config) {
        if (isPlaying()) {
            sendMessage(new PFSeqMessage(MESSAGE_TYPE_ERROR, "Cannot set up sequencer while playing"));
            if (this.listener != null)
                return false;
        }

        if (!config.isValid()) {
            sendMessage(new PFSeqMessage(MESSAGE_TYPE_ERROR, "Sequencer config not valid"));
            return false;
        }
        this._config = config;
    ...
    private boolean sync(long contentStartNanotime) {
        ...
        Log.d(LOG_TAG, "content start time: " + (contentStartNanotime - System.nanoTime()) / PFSeqService.NANO_PER_SECOND + " s from now");

        for (PFSeqTrack track : tracks) {
            boolean trackSyncSuccess = track.sync(contentStartNanotime);
            if (!trackSyncSuccess) {
                sendMessage(new PFSeqMessage(MESSAGE_TYPE_ALERT, "track " + track.getName() + " failed to sync"));
                return false;
            }
        }
                ...
                while (isPlaying()) {
                    ...
                    if (currentTime < referenceTime + pollingMillis) {
                        try {
                            ...
                            sendMessage(new PFSeqMessage(MESSAGE_TYPE_ERROR, "failed to sleep"));
                        }
                    }
            ...
            public void run() {
                ...
                while (isPlaying()) {
                    ...
                    if (System.currentTimeMillis() > startWaitingTime + getConfig().getInt(SYNC_TIME_OUT_MILLIS)) {
                        sendMessage(new PFSeqMessage(MESSAGE_TYPE_ERROR, "timeout reached"));
                        return;
                    }
                    final PFSeqTrack track = tracks.get(i);
...
//                        Log.d(LOG_TAG, "written out " + ((soonestWritableNano - System.nanoTime()) / NANO_PER_MILLIS) + " ms");
    ...
    public void stopSelf(String msg) {
        ...
        sendMessage(new PFSeqMessage(MESSAGE_TYPE_ERROR, "Had to stop sequencer. Cause: " + msg));
        stop();
    }

    private double setBpm(double newBpm) {
...
//        Log.d(LOG_TAG, "posting write - silence only: " + silenceNano + " ns");
    ...
    public boolean addTrack(PFSeqTrack track) {
        ...
        if (!isSetUp()) {
            sendMessage(new PFSeqMessage(MESSAGE_TYPE_ALERT, "could not add track because sequencer has no bee set up"));
            return false;
        }
        if (tracks.size() == getConfig().getInt(MAX_TRACKS)) {
            sendMessage(new PFSeqMessage(MESSAGE_TYPE_ALERT, "could not add track because track limit reached"));
            return false;
        }
        if (isPlaying()) {
            sendMessage(new PFSeqMessage(MESSAGE_TYPE_ALERT, "cannot add track while playing"));
            return false;
        }
...
//        Log.d(LOG_TAG, "makeSilence() - lengthFrames: " + lengthFrames + " shortsPerFrame(): " + shortsPerFrame());
    ...
    public short[] abridge(short[] pcm, int newLengthFrames) {
        ...
        if (newLengthFrames > oldLengthFrames) {
            sendMessage(new PFSeqMessage(MESSAGE_TYPE_ERROR, "can't abridge clip to longer length"));
            return pcm;
        }
...
//        Log.d(LOG_TAG, "time spent doing fade: " + (System.currentTimeMillis() - startingClock) + " ms. ");
    ...
    public int beatsSinceTempoStart(long nano) {
        if (tempoStartNanotime == null) {
            sendMessage(new PFSeqMessage(MESSAGE_TYPE_ERROR, "tempoStartNanotime not set"));
            return -1;
        }
    ...
    public int millisToFrames(int durationMillis) {
        if (!isSetUp()) {
            sendMessage(new PFSeqMessage(MESSAGE_TYPE_ERROR, "unexpected state. call to millisToFrames when seq not set up"));
            return -1;
        }
    ...
    private void sendMessage(PFSeqMessage message) {
        try {
            if (this.listener != null) {
                listener.onError(message.getType(), message.getMessage());
            }
